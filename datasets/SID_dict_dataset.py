...
class BaseDictSet():
    ...
    def __getitem__(self, index):
        ...
        if not self.load_npy:
            ...
            noisy_raw = np.load(os.path.join(self.data_path, img_file.replace('short', 'short_pack') + '.npy'),
                                allow_pickle=True)

        lbl_file = info['lbl']
        if self.load_npy:
            clean_rgb = np.load(os.path.join(self.data_path, lbl_file.replace('long', 'long_post_int') + '.npy'),
                                allow_pickle=True)
        else:
        ...
        if not self.load_npy:
            ...
            clean_raw = np.load(os.path.join(self.data_path, lbl_file.replace('long', 'long_pack') + '.npy'),
                                allow_pickle=True)
        ...
        if self.patch_size:
            ...
            if self.split == 'train':
                yy, xx = ops.randint(0, (H - patch_size) // self.block_size, (1,)), ops.randint(0, (
                            W - patch_size) // self.block_size, (1,))
            else:
            ...
            if not self.load_npy:
                ...
                clean_raw_patch = clean_raw[:, yy:yy + patch_size // self.block_size,
                                  xx:xx + patch_size // self.block_size]

            gt_patch = clean_rgb[:, yy * self.block_size:yy * self.block_size + patch_size,
                       xx * self.block_size:xx * self.block_size + patch_size]

            if self.h_flip and ops.randint(0, 2, (1,)) == 1 and self.split == 'train':  # random horizontal flip
                input_patch = np.flip(input_patch, axis=2)
                gt_patch = np.flip(gt_patch, axis=2)
                clean_raw_patch = np.flip(clean_raw_patch, axis=2)
            if self.v_flip and ops.randint(0, 2, (1,)) == 1 and self.split == 'train':  # random vertical flip
                input_patch = np.flip(input_patch, axis=1)
                gt_patch = np.flip(gt_patch, axis=1)
                clean_raw_patch = np.flip(clean_raw_patch, axis=1)
            if self.transpose and ops.randint(0, 2, (1,)) == 1 and self.split == 'train':  # random transpose
        ...
        noisy_raw = (np.float32(noisy_raw) - self.black_level) / np.float32(
            self.white_level - self.black_level)  # subtract the black level
        ...
        noisy_raw = mindspore.Tensor.from_numpy(noisy_raw).float()
        clean_rgb = mindspore.Tensor.from_numpy(clean_rgb).float()
        clean_raw = mindspore.Tensor.from_numpy(clean_raw).float()
...
class SonyDictSet(BaseDictSet):
    def __init__(self, data_path, image_list_file, patch_size=None, split='train', load_npy=True, raw_ext='ARW',
                 repeat=1,
        ...
        super(SonyDictSet, self).__init__(data_path, image_list_file, split=split, patch_size=patch_size,
                                          load_npy=load_npy, raw_ext=raw_ext, max_samples=max_samples,
                                          max_clip=max_clip, repeat=repeat,
                                          min_clip=min_clip, only_00=only_00, transpose=transpose, h_flip=h_flip,
                                          v_flip=v_flip, rotation=rotation, ratio=ratio)
        self.block_size = 2
...
class FujiDictSet(BaseDictSet):
    def __init__(self, data_path, image_list_file, patch_size=None, split='train', load_npy=True, raw_ext='RAF',
                 repeat=1,
                 max_samples=None, max_clip=1.0, min_clip=None, only_00=False,
                 transpose=True, h_flip=True, v_flip=True, rotation=False, ratio=True, **kwargs):
