...
class MultiStageNet(DNFBase):
    def __init__(self, f_number, *,
        ...
        self.aux_denoising_blocks = nn.CellList([
            ResidualSwitchBlock(
        ...
        self.aux_upsamples = nn.CellList([
            self.upsample_class(
                f_number * (2 ** idx),
        ...
        self.denoising_decoder_fuses = nn.CellList([
            self.decoder_fuse_class(in_channels=f_number * (2 ** idx)) for idx in range(layers - 1)
        ])

        self.aux_conv_fuse_0 = nn.Conv2d(f_number, f_number, 3, 1, 1, has_bias=True, pad_mode=self.pad_mode)
        self.aux_conv_fuse_1 = nn.Conv2d(f_number, aux_outchannel, 1, 1, 0, has_bias=True , pad_mode=self.pad_mode)

        inchannel = 3 if block_size == 1 else block_size * block_size
        self.aux_feature_conv_0 = nn.Conv2d(inchannel, f_number, 5, 1, 2, has_bias=True, pad_mode=self.pad_mode)
        self.aux_feature_conv_1 = nn.Conv2d(f_number, f_number, 5, 1, 2, has_bias=True, pad_mode=self.pad_mode)

        head = [2 ** layer for layer in range(layers)]
        self.aux_color_correction_blocks = nn.CellList([
        ...
        self.aux_downsamples = nn.CellList([
...
class DNF(DNFBase):
    def __init__(self, f_number, *,
        ...
        self.feedback_fuses = nn.CellList([
