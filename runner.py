...
def main(config):
    ...
    mindspore.set_context(device_target="GPU" , pynative_synchronize=True)
    logger.info(str(net))
    profile_forward = build_profile(config)
    profile_model(config, profile_forward, net, train_dataloader, logger)

    optimizer = build_optimizer(config['train'], net)
    lr_scheduler = build_scheduler(config['train'], len(train_dataloader))
    ...
    max_psnr = 0.0
    max_ssim = 0.0
    total_epochs = config['train']['early_stop'] if config['train']['early_stop'] is not None else config['train'][
        'epochs']

    if config.get('throughput_mode', False):
        throughput(config, train_forward, net, valid_dataloader, logger)
        return

    # set auto resume
    if config['train']['auto_resume']:
        auto_resume_path_1 = os.path.join(config['output'], 'checkpoints', 'checkpoint.ckpt')
        auto_resume_path_model = os.path.join(config['output'], 'checkpoints', 'model.ckpt')
        auto_resume_path_optimizer = os.path.join(config['output'], 'checkpoints', 'optimizer.ckpt')
        auto_resume_path_lrscheduler = os.path.join(config['output'], 'checkpoints', 'lrscheduler.ckpt')
        if os.path.exists(auto_resume_path_1) and os.path.exists(auto_resume_path_model)\
                and os.path.exists(auto_resume_path_optimizer) and os.path.exists(auto_resume_path_lrscheduler):
            config['train']['resume']['1'] = auto_resume_path_1
            config['train']['resume']['model'] = auto_resume_path_model
            config['train']['resume']['optimizer'] = auto_resume_path_optimizer
            config['train']['resume']['lrscheduler'] = auto_resume_path_lrscheduler
            logger.info(f'Auto resume: setting resume path to {auto_resume_path_1} and {auto_resume_path_model} and'
                        f'{auto_resume_path_optimizer} and {auto_resume_path_lrscheduler}')

    if config['train'].get('resume'):
        max_psnr = load_checkpoint(config, net, optimizer, lr_scheduler, logger)
        validate(config, test_forward, net, loss_list, valid_dataloader, config['train'].get('start_epoch', 0),
                 )
        if config.get('eval_mode', False):
            return

    if config['train'].get('pretrained') and (not config['train'].get('resume')):
        load_pretrained(config, net, logger)
        validate(config, test_forward, net, loss_list, valid_dataloader, config['train'].get('start_epoch', 0),
                 )
        if config.get('eval_mode', False):
            return
    ...
    start = time.time()
    lr = lr_scheduler(config['train'].get('start_epoch', 0))

    for epoch in range(config['train'].get('start_epoch', 0) + 1, total_epochs + 1):
        train_one_epoch(config, train_forward, net, loss_list, train_dataloader, optimizer, None, epoch, lr,
                        )
        if epoch % config['valid_per_epoch'] == 0 or (total_epochs - epoch) < 50:
            psnr, ssim, loss = validate(config, test_forward, net, loss_list, valid_dataloader, epoch)
            max_psnr = max(max_psnr, psnr)
            max_ssim = max(max_ssim, ssim)
            print('eval/max_psnr', max_psnr, epoch)
            print('eval/max_ssim', max_ssim, epoch)
        else:
            psnr = 0
        save_checkpoint(config, epoch, net, max_psnr, optimizer, lr_scheduler, logger, is_best=(max_psnr == psnr))
        logger.info(
            f'Train: [{epoch}/{config["train"]["epochs"]}] Max Valid PSNR: {max_psnr:.4f}, Max Valid SSIM: {max_ssim:.4f}')
        logger.info(
            f"Train: [{epoch}/{config['train']['epochs']}] Total Time {datetime.timedelta(seconds=int(time.time() - start))}")
        start = time.time()
...
def train_one_epoch(config, train_forward, net, loss_list, data_loader, optimizer, scaler, epoch, lr,
                    writer):
    net.set_train()
    ...
    for idx, data in enumerate(data_loader):
        ...
        grad_fn = mindspore.value_and_grad(train_forward,None, optimizer.parameters, has_aux=True)

        (outputs , targets) , grads = grad_fn(config , net , data)
        losses = loss_list(outputs, targets)
        loss = sum(losses)

        optimizer(grads)
        if config['train'].get('clip_grad'):
            grad_norm = ops.clip_by_norm(grads, config['train']['clip_grad'])
        else:
            grad_norm = grads
    ...
    logger.info(
        f"Train: [{epoch}/{config['train']['epochs']}] Time {datetime.timedelta(seconds=int(time.time() - start))}")
...
def validate(config, test_forward, model, loss_list, data_loader, epoch, writer):
    ...
    for idx, data in enumerate(data_loader):
        ...
        if config['testset_as_validset']:
            psnr, ssim = test_metric_cuda(config, epoch, outputs[config['test']['which_stage']],
                                          targets[config['test']['which_gt']], img_files, lbl_files)
        else:
            psnr, ssim = validate_metric(config, epoch, outputs[config['test']['which_stage']],
                                         targets[config['test']['which_gt']], img_files, lbl_files)
    ...
    logger.info(
        f'Valid: [{epoch}/{config["train"]["epochs"]}] Time {datetime.timedelta(seconds=int(time.time() - start))}')
...
def validate_metric(config, epoch, outputs, targets, image_paths, target_params=None):
    outputs = ops.clamp(outputs, 0, 1) * 255
    targets = targets * 255
    ...
    if config['test']['save_image'] and epoch % config['save_per_epoch'] == 0:
        images = ops.concat((outputs, targets), axis=3)
...
def test_metric_cuda(config, epoch, outputs, targets, image_paths, target_params=None):
    outputs = ops.clamp(outputs, 0, 1) * 255
    targets = ops.clamp(targets, 0, 1) * 255
    if config['test']['round']:
        outputs = outputs.round()
        targets = targets.round()
...
if __name__ == '__main__':
    ...
    current_cuda_device_target = mindspore.get_context('device_target')
    current_cuda_device_id = mindspore.get_context('device_id')
    logger.info(
        f"Current CUDA Device: {current_cuda_device_target} id: {current_cuda_device_id})")
    main(config)
